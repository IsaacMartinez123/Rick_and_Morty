...
describe('Test de RUTAS', () => {
    ...
    describe('GET /rickandmorty/login', () => {
        ...
        it('Responde con la propiedad access en true si la informacion del usuario es valida', async () => {
            const response = await request.get('/rickandmorty/login?email=isaac1@gmail.com&password=1234567')
            expect(response.body).toEqual(access)
        });

        it('Responde con la propiedad access en false si la informacion del usuario no es valida', async () => {
            const response = await request.get('/rickandmorty/login?email=isaac5@gmail.com&password=12ery567')
            access.access = false
            expect(response.body).toEqual(access)
        });
    });
